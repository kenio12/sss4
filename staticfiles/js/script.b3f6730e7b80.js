...
// コメントの既読状態を切り替える関数
function toggleReadStatus(commentId, isRead, novelId) { // novelIdを引数に追加
    ...
    .then(data => {
        if (data.success) {
            // 未読コメント数を更新
            updateUnreadCommentsCount(novelId); // 小説IDを渡して未読コメント数を更新
            // 他に未読コメントがある小説があるかチェック
            checkForOtherUnreadComments(novelId);
        } else {

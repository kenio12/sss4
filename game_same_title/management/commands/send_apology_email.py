...
10月の同タイトルイベントでは、最初に投稿されたタイトルが
その月の同タイトルになります。
テキサス万歳さんが「カセットテープ」というタイトルで
最初に投稿されたので、「カセットテープ」が10月の同タイトルに決定しました。
その決定通知を10月中旬に全会員に送信しました。
...
なぜ皆さんにこのメールが送られたかというと：
10月中旬に一部の方へ「一番槍決定」のメール送信が失敗しました。
その失敗した方々に10月21日に1回だけ再送信する処理をわしが作成しました。
ところが、わしの設定ミスで、その処理が毎日実行される設定になっていました。
そのため、本日も誤って皆さんに古い「一番槍決定」メールを送信してしまいました。

繰り返しますが、皆さんには何の関係もない話です。
